...
public class K8StackdriverReporter extends StackdriverReporter {
    ...
    protected Map<LabelKey, LabelValue> getPodLabels(String projectId) {
        ...
        if (value != null) {
            labels.put(LabelKey.create("pod_name", "Pod name"), LabelValue.create(value));
        }

        value = getPodInfo("namespace");
        if (value != null) {
            labels.put(LabelKey.create("namespace_name", "Namespace"), LabelValue.create(value));
        }

        if (projectId != null) {
            labels.put(LabelKey.create("project_id", "ProjectId"), LabelValue.create(projectId));
        }

        value = getPodInfo("cluster_name");
        if (value != null) {
            labels.put(LabelKey.create("cluster_name", "cluster_name"), LabelValue.create(value));
        }

        value = getPodInfo("cluster_location");
        if (value != null) {
            labels.put(LabelKey.create("location", "Cluster location"), LabelValue.create(value));
        }
    ...
    public void start(MetricRegistry registry, MetricFilter filter, Set<MetricAttribute> deniedExpansions) {
        ...
        if (forceResourceType != null) {
            resource = MonitoredResource.newBuilder()
                                        .setType(forceResourceType)
                                        .putAllLabels(getLabelsAsMap(projectId))
                                        .build();
        }
